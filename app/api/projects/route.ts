...
export async function GET() {
  ...
  // Group sessions by project_path; auto-generated/orphan sessions go to a special bucket
  const HORS_PROJET = '__hors_projet__'
  const byPath = new Map<string, typeof sessions>()
  for (const s of sessions) {
    const pp = s.project_path ?? ''
    const key = isAutoGeneratedProject(pp) ? HORS_PROJET : pp
  ...
  for (const [projectPath, sessionList] of byPath.entries()) {
    const isHorsProjet = projectPath === HORS_PROJET
    const slug = isHorsProjet
      ? 'hors-projet'
      : (pathToSlugMap.get(projectPath) ?? projectPath.replace(/\//g, '-'))
    ...
    // Impact: same Opus-4-6 baseline + US grid as default, consistent with cost estimate.
    const impact = sessionList.reduce(
      (acc, s) => {
        ...
        return {
          energy_wh: acc.energy_wh + i.energy_wh,
          water_ml: acc.water_ml + i.water_ml,
          co2_g: acc.co2_g + i.co2_g,
        }
      },
    ...
    projects.push({
      slug,
      project_path: isHorsProjet ? '' : projectPath,
      display_name: isHorsProjet ? 'Hors projet' : projectDisplayName(projectPath),
